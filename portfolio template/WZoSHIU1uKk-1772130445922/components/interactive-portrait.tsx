...
export default function InteractivePortrait() {
  ...
  useEffect(() => {
    ...
    scene.background = new THREE.Color(0x0a0a0a) // Dark tech background
    ...
    bgPlaneMaterial.onBeforeCompile = (shader) => {
      ...
      `.replace(
        ...
        // Subtle tech-themed background texture
        vec3 colorBg = vec3(0.04, 0.04, 0.045);  // Very dark, almost black
        vec3 colorLine = vec3(0.06, 0.08, 0.12);  // Barely visible tech blue
        ...
        // Very subtle lines
        float linePattern = fract(n * 25.0);
        float lineMix = 1.0 - smoothstep(0.48, 0.52, linePattern);
        lineMix *= 0.15; // Make lines very faint
        
        vec3 finalColor = mix(colorBg, colorLine, lineMix);
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 w-full h-full bg-gray-950 cursor-crosshair overflow-hidden"
      style={{ touchAction: "none" }}
    >
      <div className="absolute bottom-4 left-4 z-10 pointer-events-none bg-gradient-to-r from-blue-500/20 to-purple-500/20 backdrop-blur-sm border border-blue-500/30 rounded-lg px-4 py-2">
        <p className="text-blue-300 text-sm font-semibold">Interactive Portrait - Hover to reveal</p>
      </div>
    </div>
  )
}
