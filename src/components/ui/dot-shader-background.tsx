...
function Scene({ 
  dotColor = '#3b82f6',  // Blue color matching your theme
  bgColor = '#0f172a',   // Dark slate matching your background
  dotOpacity = 0.08 
}: SceneProps) {
  ...
  useEffect(() => {
    dotMaterial.uniforms.dotColor.value.setHex(dotColor.replace('#', '0x') as any)
    dotMaterial.uniforms.bgColor.value.setHex(bgColor.replace('#', '0x') as any)
...
export interface DotShaderBackgroundProps {
  dotColor?: string
  bgColor?: string
  dotOpacity?: number
  className?: string
...
export const DotShaderBackground = ({ 
  dotColor = '#3b82f6',      // Blue-400 from your theme
  bgColor = '#0f172a',       // Slate-900 from your theme  
  dotOpacity = 0.08,
  className = ''
